...
public class SimpleCart extends RandomizableClassifier implements AdditionalMeasureProducer, TechnicalInformationHandler {
	...
	protected void makeTree(Instances data, int totalInstances, int[][] sortedIndices, double[][] weights,
		...
		for (int i = 0; i < data.numAttributes(); i++) {
			...
			if (att.isNumeric()) {
				// numeric attribute
				splits[i] = numericDistribution(props, dists, att, sortedIndices[i], weights[i], totalSubsetWeights, giniGains,
						data);
			} else {
				// nominal attribute
				splitString[i] = nominalDistribution(props, dists, att, sortedIndices[i], weights[i], totalSubsetWeights,
						giniGains, data, useHeuristic);
			}
		}
		...
		if (totalWeight < 2 * minNumObj || giniGains[attIndex] == 0 || props[attIndex][0] == 0 || props[attIndex][1] == 0) {
			makeLeaf(data);
		}
			...
			for (int i = 0; i < 2; i++) {
				...
				m_Successors[i].makeTree(data, m_totalTrainInstances, subsetIndices[i], subsetWeights[i], dists[attIndex][i],
	...
	protected String toString(int level) {
		...
		if (m_Attribute == null) {
			if (Instance.isMissingValue(m_ClassValue)) {
				...
				double wrongNum = (int) ((Utils.sum(m_Distribution) - m_Distribution[Utils.maxIndex(m_Distribution)]) * 100) / 100.0;
	...
	public int getCorrectCount() {
		if (m_Attribute == null) {
			...
			return (int) ((m_Distribution[Utils.maxIndex(m_Distribution)] * 100) / 100.0);
		}
		return 0;
	}
	...
	public int getInCorrectCount() {
		if (m_Attribute == null) {
			...
			return (int) (((Utils.sum(m_Distribution) - m_Distribution[Utils.maxIndex(m_Distribution)]) * 100) / 100.0);
		}
		return 0;
	}
	 ...
	 * @return
	 */
	public String getChildCondition(int childIndex) {
		if (childIndex == 0) {
			if (m_Attribute.isNumeric())
				return " < " + m_SplitValue;
			else
	 ...
	 * @return
	 */
	public SimpleCart getChild(int childIndex) {
		if (m_Attribute == null) {
			return null;
		}
		return this.m_Successors[childIndex];
	}
