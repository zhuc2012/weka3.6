...
implements OptionHandler, WeightedInstancesHandler, 
  ...
  public int[] getClassDistribution(){
  	DiscreteEstimator est = (DiscreteEstimator)m_ClassDistribution;
  	int[] distributiton = new int[est.getNumSymbols()];
  	for(int i=0,l=est.getNumSymbols();i<l;i++){
  		distributiton[i] = (int)est.getCount(i);
  	}
  	return distributiton;
  }
  
	/**
	 * 给定一个属性的属性值，计算这个属性值在某个类别值上的数量。
	 * @param attrIndex----属性在Instance中的索引
	 * @param classIndex----分类值的索引
	 * @param attrValueIndex----属性值在这个属性名上的索引
	 * @return
	 */
	public int getCountByAttrValue(int attrIndex, int classIndex, int attrValueIndex) {
		Attribute attribute = m_Instances.attribute(attrIndex);
		int type = attribute.type();
		switch (type) {
			case Attribute.NUMERIC:
				if (m_UseKernelEstimator) {
					KernelEstimator est =  (KernelEstimator)m_Distributions[attrIndex][classIndex] ;
//					est.get
			  } else {
			  	NormalEstimator est = (NormalEstimator)m_Distributions[attrIndex][classIndex] ;
			  }
				break;
			case Attribute.NOMINAL:
				DiscreteEstimator est = (DiscreteEstimator)m_Distributions[attrIndex][classIndex];
				break;
			default:
				throw new RuntimeException("Attribute type unknown to NaiveBayes");
		}
		return 0;
	}
}
